...
function cleanDatabase() {
  ...
  while ($row = $statement->fetch()) {
    ...
    $q = $db->prepare($sql);
    $q->execute([':standard_spelling' => $standard_spelling, ':definition' => $definition, ':sample_sentence' => $sample_sentence, ':english_equivalent' => $english_equivalent, ':name' => $name, ':id' => $id]);
    $inc++;
  }
  echo $inc;
  ...
  while ($row = $statement->fetch()) {
    ...
    $q = $db->prepare($sql);
    $q->execute([':content' => $content, ':id' => $id]);
    $inc++;
  }
  echo $inc;
}
...
function clean_sentence($raw, $genre, $db) {
  ...
  if (isset($language[$genre])) {
    if (in_array($language[$genre]['name'], ['Poem', 'Song'])) {
      $raw = str_replace('.<br />', '<br />', $raw);
      $raw = str_replace('<br />', '. ', $raw);
    }
  }
  $search = ['&lsquo;', '&rsquo;', '&ldquo;', '&rdquo;', '&mdash;'];
  $fix = ["'", "'", '"', '"', '-'];
...
function check_language_permission($item_language, $db) {
  ...
  if (isset($_SESSION['permissions'])) {
    if (in_array($language_permission, $_SESSION['permissions'])) {
      return TRUE;
    }
  }
  else {
...
function fix_spelling($db) {
  ...
  foreach ($spelling as $key => $value) {
    ...
    $q = $db->prepare($sql);
    $q->execute([':standard_spelling' => $value, ':name' => $key, ':language' => '24']);
...
function fix_blacklist($db) {
  ...
  foreach ($blacklist as $value) {
    ...
    $q = $db->prepare($sql);
    $q->execute([':blacklist' => '1', ':name' => $value, ':language' => '24']);
...
function get_frequent_words($language, $db) {
  ...
  if ($frequent_manual != '') {
    $manual = unserialize($frequent_manual);
  }
  if ($frequent_generated != '') {
    $generated = unserialize($frequent_generated);
  }
...
function get_name($id, $type, $db) {
  if ($id == 'all') {
    ...
    $sql = "SELECT id,name FROM " . $type . " WHERE id =" . $id . " LIMIT 1";
  }
  $statement = $db->prepare($sql);
  $statement->execute([]);
  while ($row = $statement->fetch()) {
...
function import_semantic($db) {
  ...
  foreach ($words as $key => $value) {
    if (($value['language'] == '24') && (in_array($value['word'], $existential))) {
      ...
      $q = $db->prepare($sql);
      $q->execute([':domain' => $value['domain'], ':englishword' => $value['english'], ':name' => $value['word']]);
      // Echo 'updated<br />';.
    }
    else {
      $sql = 'INSERT INTO word (name,language,english_equivalent,domain) VALUES (:name,:language,:english_equivalent,:domain)';
      $q = $db->prepare($sql);
      $q->execute([':name' => $value['word'], ':language' => $value['language'], ':domain' => $value['domain'], ':english_equivalent' => $value['english']]);
      // Echo 'inserted<br />';.
    }
  }
}
...
function insert_article($name, $content, $db) {
  ...
  $q = $db->prepare($sql);
  $q->execute([':name' => $name, ':content' => $content]);
...
function insert_basic($type, $content, $db) {
  ...
  $q = $db->prepare($sql);
  $q->execute([':name' => $content]);
  if ($type == 'language') {
    $sql = 'INSERT INTO permissions (name,url) VALUES (:name,:url)';
    $q = $db->prepare($sql);
    $q->execute([':name' => $content, ':url' => 'language']);
  }
  header('Location: ./edit.php?type=' . $type . '&id=all');
  exit();
}
...
function insert_word($name, $language, $definition, $pos, $postwo, $sample_sentence, $english_equivalent, $domain, $blacklist, $englishword, $db) {
  ...
  $q = $db->prepare($sql);
  $q->execute([':name' => $name, ':language' => $language, ':definition' => $definition, ':pos' => $pos, ':postwo' => $postwo, ':sample_sentence' => $sample_sentence, ':english_equivalent' => $english_equivalent, ':domain' => $domain, ':blacklist' => $blacklist, ':englishword' => $englishword]);
}
...
function process_text($id, $db, $type, $name, $content, $language, $author, $year, $genre, $action) {
  ...
  if ($action == 'Add') {
    ...
    $q = $db->prepare($sql);
    $q->execute([':name' => $name, ':content' => $content, ':language' => $language, ':author' => $author, ':year' => $year, ':genre' => $genre, ':sentence_count' => $sentence_count, ':word_list' => $wordlist, ':word_count' => $word_count, ':words_per_sentence' => $words_per_sentence]);
  ...
  elseif ($action == 'Update') {
    ...
    foreach ($new_words as $word) {
      ...
      if (empty($word_array_list[$word])) {
        $word_array_list[$word] = 0;
      }
      if (empty($old_word_array[$word])) {
        $old_word_array[$word] = 0;
      }
    ...
    $q = $db->prepare($sql);
    $q->execute([':name' => $name, ':content' => $content, ':language' => $language, ':genre' => $genre, ':author' => $author, ':year' => $year, ':sentence_count' => $sentence_count, ':word_list' => $wordlist, ':word_count' => $word_count, ':words_per_sentence' => $words_per_sentence, ':id' => $id]);
...
function select_frequent_words($language, $offset, $limit, $order, $english_loan, $blacklist, $db) {
  ...
  if ($language == '0') {
    $language_condition = 'language NOT IN';
  }
  ...
  if (isset($result)) {
    ...
    echo 'No hits match your criteria.';
  }
}
...
function select_texts($language, $genre, $offset, $limit, $order, $readability, $filter, $db) {
  ...
  elseif ($order == 'wordsz') {
    $order = 'word_count ASC';
  }
  else {
    $order == 'name ASC';
  }
  if ($readability != '') {
    $readability = ' AND readability=' . $readability . ' ';
  }
  if ($language == '0') {
    $language_condition = 'language NOT IN';
  }
  if ($genre == '0') {
    $genre_condition = 'genre NOT IN';
  }
  if ($language == 'all' || $language == '0') {
    $language_array = get_name('all', 'language', $db);
    $keys = array_keys($language_array);
...
function search($word, $language, $db) {
  // Get number of search results allowed.
  $results = select_single_value('meta', '8', 'content', $db);
  ...
  if ($language != 'all') {
    ...
    $parameters = [':word' => $word, ':language' => $language];
  }
  $q = $db->prepare($sql);
  ...
  // No results; check if there are results in the "English Equivalent column".
  if ($inc < '1') {
    if ($language == 'all') {
      $sql = 'SELECT * FROM word WHERE english_equivalent ="' . $word . '"';
    }
    else {
      $sql = 'SELECT * FROM word WHERE english_equivalent ="' . $word . '" AND language =' . $language;
    }
    $q = $db->prepare($sql);
    $q->execute([]);
    while ($row = $q->fetch()) {
  ...
  // Still no results; Check for nonstandard spellings.
  if ($inc < '1') {
    $standard = '';
  ...
  // There are results.
    ...
    if (isset($exact[2]) && ($exact[1]['language'] != $exact[2]['language'])) {
      ...
      if ($exact[1]['definition'] != '') {
        echo '<br /><b>Meaning: </b>' . $exact[1]['definition'] . '<br />';
      }
      elseif ($exact[1]['english_equivalent'] != '' && $exact[1]['english_equivalent'] != '0' && $exact[1]['language'] != '25') {
        echo '<br /><b>English Equivalent: </b>' . $exact[1]['english_equivalent'] . '<br />';
      }
      ...
      if ($exact[1]['english_equivalent'] != '0' && $exact[1]['english_equivalent'] != '') {
        ...
        $q = $db->prepare($sql);
        $q->execute([':english_equivalent' => $exact[1]['english_equivalent'], ':language' => $exact[1]['language']]);
        $already = [];
        $first = 0;
        while ($row = $q->fetch()) {
  ...
  if (isset($_POST['submit'])) {
    ...
    if ($_POST['submit'] == 'Find Phrases') {
      ...
      if ($_POST['language'] == 'all') {
        ...
        $arguments = [':word' => '% ' . $word . ' %', ':language' => $_POST['language']];
      }
...
function statistical_analysis_computations($result, $db, $language_id) {
  if (isset($result)) {
    ...
    foreach ($result as $key => $value) {
      ...
      $total_categorized++;
      if ($value['pos'] == '0') {
        $major['uncategorized'][] = $value['name'];
        $total_categorized = $total_categorized - 1;
      }
      elseif ($clitic !== FALSE) {
        $major['clitic'][] = $value['name'];
      }
      elseif ($linker !== FALSE) {
        $major['linker'][] = $value['name'];
      }
      elseif ($pronoun !== FALSE) {
        $major['pronoun'][] = $value['name'];
      }
      elseif ($determiner !== FALSE) {
        $major['determiner'][] = $value['name'];
      }
      elseif ($numeral !== FALSE) {
        $major['numeral'][] = $value['name'];
      }
      else {
        $major[$posone][] = $value['name'];
      }
    ...
    if (isset($major)) {
      ...
      $output .= '<br />No words are tagged with parts of speech in this language';
    }
  }
  else {
    $output = '<br />No words were found in this language';
  }
  return $output;
}
...
function update_article($id, $name, $content, $db) {
  ...
  $q = $db->prepare($sql);
  $q->execute([':name' => $name, ':content' => $content, ':id' => $id]);
}
...
function update_basic($type, $id, $content, $db) {
  if ($type == 'language') {
    ...
    $q = $db->prepare($sql);
    $q->execute([':name' => $content, ':old' => $name]);
  }
  $sql = 'UPDATE ' . $type . ' SET name = :name WHERE id = :id';
  $q = $db->prepare($sql);
  $q->execute([':name' => $content, ':id' => $id]);
}
...
function update_frequent_words($language, $db) {
  ...
  if ($total_words >= $limit) {
    ...
    $statement = $db->prepare($sql);
    $statement->execute([$language, 0, 0]);
    while ($row = $statement->fetch()) {
      $frequent[] = $row['name'];
    }
    ...
    $q = $db->prepare($sql);
    $q->execute([':frequent_words' => $frequent_words, ':id' => $language]);
  }
}
...
function update_distinct_words($language, $db) {
  ...
  $q = $db->prepare($sql);
  $q->execute([':distinct_words' => $distinct, ':language' => $language]);
}
...
function update_language($id, $frequent_word_value, $sentences_constant, $words_constant, $db) {
  ...
  $q = $db->prepare($sql);
  $q->execute([':frequent_word_value' => $frequent_word_value, ':sentences_constant' => $sentences_constant, ':words_constant' => $words_constant, ':id' => $id]);
}
...
function update_readability($language, $id, $db) {
  ...
  if ($frequent_words != '') {
    ...
    $q = $db->prepare($sql);
    $q->execute([':readability' => $readability, ':id' => $id]);
  }
}
...
function update_total_words($word_count, $language, $db) {
  ...
  $q = $db->prepare($sql);
  $q->execute([':value' => $words, ':id' => $language]);
}
