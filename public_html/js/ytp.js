    ...
    if (list.length > 1) {
        ...
        for (var i=0; i<list.length; i++)
        {
            ...
            listLink.onclick = function () {
                ytLoadNewVideo(pid, this.id, this.firstChild.nodeValue)};
...
function ytPlayerDispatch(ytp,ytpid) 
{
    if (ytp) {
        ...
        setInterval(
            function () {
                updateButtonState(ytpid); }, 250
        );
        setInterval(
            function () {
                updateTime(ytpid); }, 500
        );
    }
}
...
function ytloop(ytpid)
{
    ...
    if (new RegExp('^(' + ytplayer.join('|') + ')$').test(ytp.id)) {
        ...
        if (loopbut.firstChild.nodeValue == "Loop") {
            ...
            ytLoopInterval[ytpid] = window.setInterval(
                function () {
                    ytp.playVideo(); }, 2000
            );
        }
