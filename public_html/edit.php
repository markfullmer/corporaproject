...
if ($type == 'meta') {
  check_permissions($permission = [10]);
}
if ($type == 'genre') {
...
if ($type == 'text' && (!in_array($id, ['add', 'all']))) {
...
<article>
<?php
if ($type == 'language' && $id == 'all') {
  echo '<div class="message">If you delete a language, any texts or words already in the database will remain, but they will be listed as "Uncategorized".</div>';
}
...
if ($id == 'all') {
    ...
    foreach ($data as $key => $value) {
      if ($type == 'user' && $key == '1' && $_SESSION['uid'] != '1') {
        ...
        if ($type == 'text') {
          ...
          if ($type != 'article' && !in_array($id, [1, 2, 3])) {
            echo ' (<a href="save.php?type=' . $type . '&id=' . $key . '&submit=delete">delete</a>)';
          }
          echo '<br />';
        }
      }
  ...
  if ($type == 'user' && $id == '1' && $_SESSION['uid'] != '1') {
    die('You do not have permission to access this.');
  }
  $data = get_all($type, $id, $db);
  if (in_array($type, ['language', 'domain', 'genre', 'word', 'pos'])) {
    $content = $data['name'];
  }
  else {
    $content = br2nl($data['content']);
  }

  if ($type == 'user') {
    if ($data['access'] != '') {
      $p = unserialize($data['access']);
      $perms = array_keys($p);
    }
    else {
      $p = ['0'];
  ...
  if (in_array($type, ['user', 'article'])) {
    echo '<input type="text" name="name" value="' . $data['name'] . '" style="width:300px" placeholder="Name"/><br />';
  }
  if (in_array($type, ['text', 'article'])) {
    echo '<textarea name="content" style="width:100%;height:400px;">' . $content . '</textarea>';
  }
  if (!in_array($type, ['user', 'meta', 'article', 'text'])) {
  ...
  if ($type == 'meta') {
    ...
    while ($row = $statement->fetch()) {
      ...
      if ($row['id'] == '6') {
        $words_to_display = $row['content'];
      }
      if ($row['id'] == '7') {
        $texts_to_display = $row['content'];
      }
      if ($row['id'] == '8') {
        $search_results = $row['content'];
      }
    }
...
<span id="progress" class="hide"><span id="finished">0</span> out of ' . $total . '</span>';
    ...
    if (isset($data['frequent_words'])) {
      ...
      if (isset($words[0])) {
        ...
        echo 'There are not enough words yet to generate a frequent word list.';
      }
      echo '</div>';
    ...
    if (!empty($data['frequent_manual'])) {
      ...
      echo 'There are no manually added words yet.';
    }
    echo '</div>';
...
<aside>
  ...
  if (isset($data['sentence_count'])) {
    echo '<p>Sentences: ' . $data['sentence_count'] . '<p>';
  }
